...
if(isset($_POST[submit]))
{
	...
	{
		$sql ="INSERT INTO prescription_records(prescription_id,medicine_name,cost,unit,dosage,status) values('$_POST[prescriptionid]','$_POST[medicineid]','$_POST[cost]','$_POST[unit]','$_POST[select2]','In Stock')";
		if($qsql = mysqli_query($con,$sql))
		{	
...
<section id="page-header" style="margin-top:100px;">
    <div class="row text-center">
        ...
        <div class="col-md-4">
            <h2 class="h2w">Add Prescription</h2>
        </div>
...
<section id="login_page">
	...
	<hr>
<center><h2>Add Prescription record</h2></center>
           <?php
			if(!isset($_SESSION[patientid]))
			{
		  ?>  
<section id="login_page">
    <div class="container" style="padding:0px 25px 20px 50px;">
        <div class="card" style="margin:auto;width:60%">
            <div class="card-body" style="margin:auto;margin-top:10px;width:90%">
  ...
  <input type="hidden" name="prescriptionid" value="<?php echo $_GET[prescriptionid]; ?>"  />
    <table width="100%" style="color:white;font-size:20px">
        ...
        <tr>
		  ...
		  <?php
		$sqlmedicine ="SELECT * FROM medicine WHERE status='In Stock'";
		$qsqlmedicine = mysqli_query($con,$sqlmedicine);
		while($rsmedicine = mysqli_fetch_array($qsqlmedicine))
		{
			echo "<option value='$rsmedicine[medicinename]'>$rsmedicine[medicinebrand] ( ₹ $rsmedicine[medicinecostad] )</option>";
		}
		?>
		  </select>
		  </td>
...
<center><h2>View Prescription record</h2></center>
<div id="printpart">
<section id="login_page">
    <div class="container" style="padding:0px 25px 20px 50px;">
        <div class="card" style="margin:auto;width:60%">
        ...
        <tr style="border-bottom:1px solid white;">
         ...
         <?php
		 $gtotal=0;
        $sql ="SELECT * FROM prescription_records LEFT JOIN medicine on prescription_records.medicine_name=medicine.medicinename WHERE prescription_id='$_GET[prescriptionid]'";
		$qsql = mysqli_query($con,$sql);
		while($rs = mysqli_fetch_array($qsql))
		{
        ...
        <tr>
          <td colspan="6"><br><div align="center">
              <input type="submit" class="btn btn-outline-primary" name="print" id="print" value="Print" onclick="myFunction('printpart')"/>
          </div></td>
          </tr>
      </tbody>
    </table>
